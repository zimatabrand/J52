...
-- ============================================
-- J5 SCHEMA: All J52 tables
-- ============================================
CREATE SCHEMA IF NOT EXISTS j5;

-- Schema registry - tracks all project schemas
CREATE TABLE IF NOT EXISTS j5.schema_registry (
    schema_code     VARCHAR(9) PRIMARY KEY,
    parent_schema_code VARCHAR(9) REFERENCES j5.schema_registry(schema_code),
...
CREATE INDEX idx_schema_registry_parent ON j5.schema_registry(parent_schema_code);
CREATE INDEX idx_schema_registry_active ON j5.schema_registry(is_active) WHERE is_active = TRUE;

-- Projects - one per schema
CREATE TABLE IF NOT EXISTS j5.projects (
    project_id      SERIAL PRIMARY KEY,
    schema_code     VARCHAR(9) NOT NULL UNIQUE REFERENCES j5.schema_registry(schema_code) ON DELETE CASCADE,
...
CREATE INDEX idx_projects_schema ON j5.projects(schema_code);
CREATE INDEX idx_projects_active ON j5.projects(is_active) WHERE is_active = TRUE;

-- AI sessions - conversation sessions across projects
CREATE TABLE IF NOT EXISTS j5.ai_sessions (
    session_id      SERIAL PRIMARY KEY,
    schema_code     VARCHAR(9) REFERENCES j5.schema_registry(schema_code) ON DELETE SET NULL,
    parent_session_id INTEGER REFERENCES j5.ai_sessions(session_id),
...
CREATE INDEX idx_sessions_schema ON j5.ai_sessions(schema_code);
CREATE INDEX idx_sessions_status ON j5.ai_sessions(status);
CREATE INDEX idx_sessions_active ON j5.ai_sessions(status) WHERE status = 'active';

-- Chat I/O log - messages within sessions
CREATE TABLE IF NOT EXISTS j5.chat_io_log (
    log_id          SERIAL PRIMARY KEY,
    session_id      INTEGER NOT NULL REFERENCES j5.ai_sessions(session_id) ON DELETE CASCADE,
...
CREATE INDEX idx_chat_io_session ON j5.chat_io_log(session_id);
CREATE INDEX idx_chat_io_session_seq ON j5.chat_io_log(session_id, sequence_number);

-- Tasks - project tasks and to-dos
CREATE TABLE IF NOT EXISTS j5.tasks (
    task_id         SERIAL PRIMARY KEY,
    schema_code     VARCHAR(9) REFERENCES j5.schema_registry(schema_code) ON DELETE SET NULL,
    session_id      INTEGER REFERENCES j5.ai_sessions(session_id) ON DELETE SET NULL,
    parent_task_id  INTEGER REFERENCES j5.tasks(task_id),
...
CREATE INDEX idx_tasks_schema ON j5.tasks(schema_code);
CREATE INDEX idx_tasks_status ON j5.tasks(status);
CREATE INDEX idx_tasks_parent ON j5.tasks(parent_task_id);
CREATE INDEX idx_tasks_assigned ON j5.tasks(assigned_to) WHERE assigned_to IS NOT NULL;

-- User settings - key-value pairs per user
CREATE TABLE IF NOT EXISTS j5.user_settings (
...
CREATE TABLE IF NOT EXISTS j5.memory_facts (
...
CREATE INDEX idx_memory_category ON j5.memory_facts(category);

-- Audit log - tracks important system events
CREATE TABLE IF NOT EXISTS j5.audit_log (
    log_id          SERIAL PRIMARY KEY,
    user_id         VARCHAR(100),
...
CREATE INDEX idx_audit_action ON j5.audit_log(action);
CREATE INDEX idx_audit_resource ON j5.audit_log(resource_type, resource_id);
CREATE INDEX idx_audit_created ON j5.audit_log(created_at);

-- Session summaries - AI-generated session digests
CREATE TABLE IF NOT EXISTS j5.session_summaries (
...
CREATE INDEX idx_summaries_session ON j5.session_summaries(session_id);

-- AI providers - registry of available AI tools
CREATE TABLE IF NOT EXISTS j5.ai_providers (
...
INSERT INTO j5.schema_migrations (version, name) VALUES (1, '001_initial_schema');

-- ============================================
-- FUNCTION: Create per-project schema
-- ============================================
CREATE OR REPLACE FUNCTION j5.create_project_schema(
...
BEGIN
    ...
    INSERT INTO j5.schema_registry (
...
-- ============================================
CREATE OR REPLACE FUNCTION j5.update_timestamp()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
CREATE TRIGGER trg_schema_registry_updated BEFORE UPDATE ON j5.schema_registry
    FOR EACH ROW EXECUTE FUNCTION j5.update_timestamp();
CREATE TRIGGER trg_projects_updated BEFORE UPDATE ON j5.projects
    FOR EACH ROW EXECUTE FUNCTION j5.update_timestamp();
CREATE TRIGGER trg_tasks_updated BEFORE UPDATE ON j5.tasks
    FOR EACH ROW EXECUTE FUNCTION j5.update_timestamp();
CREATE TRIGGER trg_memory_facts_updated BEFORE UPDATE ON j5.memory_facts
    FOR EACH ROW EXECUTE FUNCTION j5.update_timestamp();
CREATE TRIGGER trg_user_settings_updated BEFORE UPDATE ON j5.user_settings
    FOR EACH ROW EXECUTE FUNCTION j5.update_timestamp();

-- ============================================
-- Seed data: Default AI provider
-- ============================================
INSERT INTO j5.ai_providers (provider_name, display_name, launch_command, model_name)
